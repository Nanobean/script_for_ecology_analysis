...
for aou in tqdm(range(len(arbigallcorr))):
    ...
    aouoklist.append(toklist)
#距离分类完成，接下来是输出

birdsort=ifelse
def filename(x):
    return str(birdsort[x])+'.xlsx'

os.chdir('D:\python\50aou')
import xlsxwriter
for i in tqdm(range(len(birdsort))):
